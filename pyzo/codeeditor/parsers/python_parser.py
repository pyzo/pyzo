...
stringLiteralPrefixesF = [p for p in stringLiteralPrefixes if ("f" in p or "t" in p)]
...
class Python3Parser(RegexParser):
    ...
    states = {
        ...
        "soft-keywords-inner": [
            # optional `_` keyword
            (r"(\s+)(?:[^\n_]*)(_\b)", bygroups(WhitespaceToken, KeywordToken)),
            default("#pop"),
        ],
        "string": [
        ...
        "fstring-multiline-dispatcher": [
            (
                rf"({'|'.join(stringLiteralPrefixesF)})?(''')",
                bygroups(NonIdentifierToken, MultilineFStringToken),
        ...
        "fstring-oneline-dispatcher": [
            (
                rf"({'|'.join(stringLiteralPrefixesF)})?(')",
                bygroups(NonIdentifierToken, FStringToken),
